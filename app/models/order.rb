class Order
  ...
  with_options presence: true do
    validates :token
    validates :buyer_postal_code, format: { with: /\A\d{3}-\d{4}\z/, message: 'はハイフン(-)を含めて半角数字で入力してください', allow_blank: true }
    validates :prefecture_id, numericality: { other_than: 1, message: 'は[---]以外を選択してください', allow_blank: true }
    validates :buyer_city, :buyer_address
    validates :buyer_phone_number, format: { with: /\A\d{10,11}\z/, message: 'は半角数字のみを用いて10桁もしくは11桁で入力してください', allow_blank: true }
